...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const currentChainHeight = await this.getChainHeight();
            ...
            try {
              ...
              if (now - messageTime < 300) {
                 if (bitcoinMessage.verify(message, address, signature)) {
                     const newBlock = await self._addBlock(new BlockClass.Block({
                         address,
                         star
                     }));
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for await (const block of self.chain) {
                ...
                if (!isValid) {
                    errorLog.push(`Block has been tampered! Block ${block.height}: ${block.hash}`);
                }

                if (block.height !== 0 && block.previousBlockHash !== self.chain[block.height - 1].hash) {
                    errorLog.push(`Chain is broken! Block ${block.height}: ${block.hash}`);
                }
            }
